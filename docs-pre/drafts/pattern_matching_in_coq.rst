...
Agda ではこういう別の箇所から分かるパターンの部分で ``Just .A ?`` と書くことも
でき、dot patterns と呼ばれています。Coq の書き方ではアンダースコア (``_``) に
無視したい場合と書けない場合の二つの意味が出てきてしまうので、Agda の方が筋が
通っているように感じます。ただ、Coq では組み立てるときの ``Just Int 1`` の
``Int`` が暗黙引数で省略できるのと同じように、パターンでの ``Just _ a`` の
``_`` も暗黙引数で省略できるので、実用上では困ったことはないです。
