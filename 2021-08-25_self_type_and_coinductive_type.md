...
自分型で余帰納型をエンコーディングする方法の考察を行います。自分型は `x : P x` のように自分自身を型付けするのに自分自身を使うものを表す型です。これを使ってチャーチ・エンコーディングのような方法で体系へ余帰納型を組み込めないか考察しています。
...
* 帰納型を体系に加える方法について。
* チャーチ・エンコーディングについて。
* ベーム・ベラルドゥッチ・エンコーディング (Böhm–Berarducci encoding) について。
* パリゴット・エンコーディング (Parigot encoding) について。
* 組み込みの帰納型について。
* 帰納型を体系に加える方法にある様々な問題について。
* それらの問題を自分型が解決できることについて。
* 余帰納型について。
* 依存型に対応した余帰納原理について。
* 自分型について。
* 自分型による帰納型のエンコーディングについて。
* 自分型による余帰納型のエンコーディングについて。
* 冪モナドについて。
...
他の帰納型についても同じようなエンコーディングが可能です。詳細は論文を読むか [Yatima 言語](https://github.com/yatima-inc/yatima)で遊んで把握するかしてください。ちなみに、このヤチマは、あのグレッグ・イーガンの『ディアスポラ』の主人公たるヤチマに由来しています。『ディアスポラ』は面白いですよ。
...
実は、これはモナドになるのです。なぜならば、 `X -> Type` は冪集合の類似物と見なすことが出来て、集合の圏において冪集合の操作は冪集合モナド (power set monad) というものになるからです。
...
`y : Stream A` を自分だと考えて自分型で閉じます。これが `Stream A` の自分型によるエンコーディングと言えましょう。
...
それはそうと、 `coinduction` は、どんな挙動をするのか考えてみましょう。余パターン (copattern) を使って表示することにします。これは、返り値に対して分解子を適用した結果で関数の定義を構築する物であり、この場合は `coinduction A P destruct_head destruct_tail y x` の `y` に対して `head` と `tail` を適用するとどうなるかを考えます。
...
`coinduction` は、最終的には `Π y : Stream A. P y -> Type` という関係を返します。これは関数を表す関係であって、 `x : P y` が定まれば `y : Stream A` が定まるというものです。なんというか、これが余依存型の正体だとしたら、前の計算で情報を余り持たないという結果が出たのも納得です。
...
さて、これを自分型で表したいところですが、やっぱり不可能です。なので、これでおしまいです。

## 自分型による余帰納型のエンコーディング（４）

余帰納型は帰納型で表現することが出来ることが証明されています。そのため、余帰納型を帰納型でエンコーディングした後に、帰納型を自分型でエンコーディングします。

## まとめ

余帰納型において依存型に関する余帰納法を表現することは可能です。たとえば、次のようになります。
...
自分型による余帰納型のエンコーディングは、冪モナドを使えば無理矢理実現することが可能です。たとえば、次のようになります。
