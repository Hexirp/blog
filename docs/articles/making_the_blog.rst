...
この成果をプッシュしたら、TravisCI のビルドがなぜかキャッシュを読み込まず、
...
実際に走らせてみたらもちろん問題が見つかりました。

第一に git pull が行われても master が取得されませんでした。
これは https://github.com/Hexirp/blog/pull/5 で修正しました。
なんでこんなことになっているのかはわかりませんでしたが、
解決策は見つけられました。詳細はその Pull Request の説明を見てください。

第二に、全てが解決しているはずなのになぜかプッシュが失敗しました。
だめもとでトークンを再暗号化したら直りました。どこかでコピペを間違えたようです。

他にもいろいろありましたが、きつかったのは上の二つです。

*******
GHC 8.4
*******

GHC 8.4 に更新した結果、色々うれしいことがありました。

何よりもうれしいのは、一番最初にぶつかったあのバグが発生しないことです！
まだ Hakyll が Windows に対応していない状況は変わっていないのが残念です。

****
清書
****

もう、ブログ作りは終わったといえます。最後に清書をしておきます。
英語と日本語の変わり目に空白を入れたりと色々細かい作業をしました。
