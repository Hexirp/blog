...
## 追記・ぎりぎりブラックホールにならないようにしたらどうなるの？

この質問は記事の発表後にグーゴロジストの社交場で okkuu さん ([@kenkenokkuu](https://twitter.com/kenkenokkuu)) から受けたものである。まず、この質問の前に「もしブラックホールを考慮しなければ O(n^(1/3)) になる」という話をしていた。そこで、 okkuu さんが「ぎりぎりブラックホールにならないようにしたらどうなるの？」という旨の質問をしてきた。

確かに、ブラックホールを考慮しなければ O(n^(1/3)) であるのならば、ぎりぎりブラックホールにならないようにすれば O(n^(1/3)) を実現できるのではないかという気がしないでもない。もちろん、そんな気がするだけで、そんな筈はないはずである。考察してみよう。

ぎりぎりブラックホールではない物体といえば中性子星である。中性子星の内部は未知であるが、そこで何らかのプロセスによって計算が行われていると仮定してみよう。時には意識でさえも産み出されるかもしれない。このような計算実体が使用できるメモリは明らかに O(n^(1/3)) に比例している。

だが、これは n を大きくしていく——つまり中性子星の半径を大きくしていくと、途端に崩壊する。中性子星の質量にはトルマン・オッペンハイマー・ヴォルコフ限界と呼ばれる上限が存在し、これを超えるとブラックホールに崩壊してしまうのだ。そして、そうすると計算実体を維持することは出来なくなってしまう。

このようなブラックホールへの崩壊を避けるためには、結局のところメモリの容量を大きくしていくにつれてメモリの密度を薄くしていくしかない。

ブラックホールが存在しないのであればランダムアクセスの計算量は O(n^(1/3)) である。しかし、ブラックホールが存在するためランダムアクセスの計算量は O(n^(1/2)) よりも効率よくすることが出来ない。この限界は、メモリの機能を保つことが出来る範囲でメモリの密度を上げるという問題においても効いてくる。

メモリの容量を大きくしていくほどメモリの密度を下げていくのであれば、ブラックホールに崩壊させないまま限界ぎりぎりを攻めていくことが出来る。このような条件を満たす現実的な機構を考えると——プロセッサが宇宙に浮かんでいる。それを球状の殻が取り囲んでいる。その内側には超高密度のメモリが配置されている。プロセッサとメモリの間で光子による通信が行われている。

まるでグレッグ・イーガンの『シルトの梯子』のような情景になってしまった。
